!function() {
  ...
  function d3_geo_clipCircle(radius) {
    ...
    function intersect(a, b, two) {
      ...
      var someSymbollambda = lambda1 - lambda0, polar = abs(someSymbollambda - pi) < epsilon, meridian = polar || someSymbollambda < epsilon;
      if (!polar && phi1 < phi0) z = phi0, phi0 = phi1, phi1 = z;
      if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (abs(q[0] - lambda0) < epsilon ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : someSymbollambda > pi ^ (lambda0 <= q[0] && q[0] <= lambda1)) {
  ...
  function d3_geo_resample(project) {
    var someSymbol2 = .5, cosMinDistance = Math.cos(30 * d3_radians), maxDepth = 16;
    ...
    function resampleLineTo(x0, y0, lambda0, a0, b0, c0, x1, y1, lambda1, a1, b1, c1, depth, stream) {
      ...
      if (d2 > 4 * someSymbol2 && depth--) {
        ...
        if (dz * dz / d2 > someSymbol2 || abs((dx * dx2 + dy * dy2) / d2 - .5) > .3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
    ...
    resample.precision = function(_) {
      if (!arguments.length) return Math.sqrt(someSymbol2);
      maxDepth = (someSymbol2 = _ * _) > 0 && 16;
      return resample;
    };
    return resample;
  }
  ...
  function d3_geo_projectionMutator(projectAt) {
    var project, rotate, projectRotate, projectResample = d3_geo_resample(function(x, y) {
      x = project(x, y);
      return [ x[0] * k + someSymbolx, someSymboly - x[1] * k ];
    }), k = 150, x = 480, y = 250, lambda = 0, phi = 0, someSymbollambda = 0, someSymbolphi = 0, someSymbolweirdY = 0, someSymbolx, someSymboly, preclip = d3_geo_clipAntimeridian, postclip = d3_identity, clipAngle = null, clipExtent = null, stream;
    function projection(point) {
      point = projectRotate(point[0] * d3_radians, point[1] * d3_radians);
      return [ point[0] * k + someSymbolx, someSymboly - point[1] * k ];
    }
    function invert(point) {
      point = projectRotate.invert((point[0] - someSymbolx) / k, (someSymboly - point[1]) / k);
    ...
    projection.rotate = function(_) {
      if (!arguments.length) return [ someSymbollambda * d3_degrees, someSymbolphi * d3_degrees, someSymbolweirdY * d3_degrees ];
      someSymbollambda = _[0] % 360 * d3_radians;
      someSymbolphi = _[1] % 360 * d3_radians;
      someSymbolweirdY = _.length > 2 ? _[2] % 360 * d3_radians : 0;
      return reset();
    };
    d3.rebind(projection, projectResample, "precision");
    function reset() {
      projectRotate = d3_geo_compose(rotate = d3_geo_rotation(someSymbollambda, someSymbolphi, someSymbolweirdY), project);
      var center = project(lambda, phi);
      someSymbolx = x - center[0] * k;
      someSymboly = y + center[1] * k;
      return invalidate();
    }
    function invalidate() {
      if (stream) stream.valid = false, stream = null;
  ...
  function d3_geo_rotation(someSymbollambda, someSymbolphi, someSymbolweirdY) {
    return someSymbollambda ? someSymbolphi || someSymbolweirdY ? d3_geo_compose(d3_geo_rotationlambda(someSymbollambda), d3_geo_rotationphiweirdY(someSymbolphi, someSymbolweirdY)) : d3_geo_rotationlambda(someSymbollambda) : someSymbolphi || someSymbolweirdY ? d3_geo_rotationphiweirdY(someSymbolphi, someSymbolweirdY) : d3_geo_identityRotation;
  }
  function d3_geo_forwardRotationlambda(someSymbollambda) {
    return function(lambda, phi) {
      return lambda += someSymbollambda, [ lambda > pi ? lambda - tau : lambda < -pi ? lambda + tau : lambda, phi ];
    };
  }
  function d3_geo_rotationlambda(someSymbollambda) {
    var rotation = d3_geo_forwardRotationlambda(someSymbollambda);
    rotation.invert = d3_geo_forwardRotationlambda(-someSymbollambda);
    return rotation;
  }
  function d3_geo_rotationphiweirdY(someSymbolphi, someSymbolweirdY) {
    var cossomeSymbolphi = Math.cos(someSymbolphi), sinsomeSymbolphi = Math.sin(someSymbolphi), cossomeSymbolweirdY = Math.cos(someSymbolweirdY), sinsomeSymbolweirdY = Math.sin(someSymbolweirdY);
    function rotation(lambda, phi) {
      var cosphi = Math.cos(phi), x = Math.cos(lambda) * cosphi, y = Math.sin(lambda) * cosphi, z = Math.sin(phi), k = z * cossomeSymbolphi + x * sinsomeSymbolphi;
      return [ Math.atan2(y * cossomeSymbolweirdY - k * sinsomeSymbolweirdY, x * cossomeSymbolphi - z * sinsomeSymbolphi), d3_asin(k * cossomeSymbolweirdY + y * sinsomeSymbolweirdY) ];
    }
    rotation.invert = function(lambda, phi) {
      var cosphi = Math.cos(phi), x = Math.cos(lambda) * cosphi, y = Math.sin(lambda) * cosphi, z = Math.sin(phi), k = z * cossomeSymbolweirdY - y * sinsomeSymbolweirdY;
      return [ Math.atan2(y * cossomeSymbolweirdY + z * sinsomeSymbolweirdY, x * cossomeSymbolphi + k * sinsomeSymbolphi), d3_asin(k * cossomeSymbolphi - x * sinsomeSymbolphi) ];
    };
    return rotation;
  }
